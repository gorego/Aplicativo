...
namespace Aplicativo
{
    public partial class frmPaqueteOP : Form
    {
        ...
        {
            InitializeComponent();
            Variables.cargar(dataGridView14, "SELECT paq.Id, paq.numPaquete, prod.Codigo, paq.Bodega, paq.numPiezas, paq.dia, (Avg(info.ancho1) + Avg(info.ancho2))/2, (Avg(info.alto1) + Avg(info.alto2))/2 FROM (Paquete AS paq INNER JOIN Productos AS prod ON paq.Producto = prod.ID) INNER JOIN infoPaquete AS info ON paq.Id = info.Paquete WHERE (((paq.OP)="+op+") AND ((paq.Producto)="+producto+")) GROUP BY paq.Id, paq.numPaquete, prod.Codigo, paq.Bodega, paq.numPiezas, paq.dia, paq.porcentaje");
