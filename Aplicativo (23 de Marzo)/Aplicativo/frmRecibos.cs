    ...
    public partial class frmRecibos : Form
    {
        ...
        public frmRecibos(int tipo)
        {
            ...
            else
            {
                ...
                Variables.cargar(dataGridView2, "SELECT reciboCliente.*, Lotes.Lote, h.OT FROM (Lotes INNER JOIN reciboCliente ON Lotes.Codigo = reciboCliente.Lote) INNER JOIN historicoOrdenes AS h ON reciboCliente.Orden = h.ID WHERE (((reciboCliente.volumenActual)>0)) ORDER BY reciboCliente.ID DESC;");
                Variables.cargar(dataGridView1, "SELECT reciboCliente.*, Lotes.Lote, h.OT FROM (Lotes INNER JOIN reciboCliente ON Lotes.Codigo = reciboCliente.Lote) INNER JOIN historicoOrdenes AS h ON reciboCliente.Orden = h.ID ORDER BY reciboCliente.ID DESC;");
                Variables.cargar(dataGridView3, "SELECT reciboCliente.*, Lotes.Lote, h.OT FROM (Lotes INNER JOIN reciboCliente ON Lotes.Codigo = reciboCliente.Lote) INNER JOIN historicoOrdenes AS h ON reciboCliente.Orden = h.ID WHERE reciboCliente.Especie = 'Melina' ORDER BY reciboCliente.ID DESC;");
                Variables.cargar(dataGridView4, "SELECT reciboCliente.*, Lotes.Lote, h.OT FROM (Lotes INNER JOIN reciboCliente ON Lotes.Codigo = reciboCliente.Lote) INNER JOIN historicoOrdenes AS h ON reciboCliente.Orden = h.ID WHERE reciboCliente.Especie = 'Teca' ORDER BY reciboCliente.ID DESC;");
                Variables.cargar(dataGridView5, "SELECT reciboCliente.*, Lotes.Lote, h.OT FROM (Lotes INNER JOIN reciboCliente ON Lotes.Codigo = reciboCliente.Lote) INNER JOIN historicoOrdenes AS h ON reciboCliente.Orden = h.ID WHERE reciboCliente.Especie <> 'Melina' AND reciboCliente.Especie <> 'Teca' ORDER BY reciboCliente.ID DESC;");
            }
        ...
        {
            if (data.CurrentCell.ColumnIndex == 26 && tipo2 == 0)
            {
            ...
            else if (data.CurrentCell.ColumnIndex == 27 && tipo2 == 1)
            {
                frmCrearOrden newFrm = new frmCrearOrden(data.Rows[data.CurrentCell.RowIndex].Cells[1].Value.ToString(), 1);
                newFrm.Show();
            }
        ...
        {
            string query = "SELECT Recibo.*, Lotes.Lote, h.OT FROM (Lotes INNER JOIN reciboCliente As Recibo ON Lotes.Codigo = Recibo.Lote) INNER JOIN historicoOrdenes AS h ON Recibo.Orden = h.ID  ";
            int i = 0;
            if (!comboBox1.Text.Equals(""))
            {
                if (i != 0)
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (button2.Text != "Modificar Modulo")
            {
                ...
                {
                    if (view == 1)
                        Variables.cargar(dataGridView1, "SELECT Recibo.*, Lotes.Lote, h.OT FROM (Lotes INNER JOIN reciboCliente as Recibo ON Lotes.Codigo = Recibo.Lote) INNER JOIN historicoOrdenes AS h ON Recibo.Orden = h.ID ORDER BY Recibo.ID DESC;");
                    else if (view == 2)
