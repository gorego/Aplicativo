...
namespace Aplicativo
{
    public partial class frmReciboMadera : Form
    {
        ...
        {
            double volumen = 0;
            string query = "SELECT Volumen FROM volumenCalculado WHERE Diametro = " + diametro.ToString().Replace(",", ".") + " AND Largo = " + largo.ToString().Replace(",", ".") + " AND Cantidad = " + cantidad + " AND Especie = '" + especie + "' AND Raleo = '" + raleo + "' AND Trailer = '" + trailer + "'";
            //Ejecutar el query y llenar el GridView.
            conn.ConnectionString = connectionString;
            OleDbCommand cmd = new OleDbCommand(query, conn);
            cmd.Connection = conn;
        ...
        public void agregarRecibo()
        {
            conn.ConnectionString = connectionString;
            OleDbCommand cmd;
            cmd = new OleDbCommand("INSERT INTO Recibo (Orden,Clasificacion,Motivo,Especie,volumenIngreso,volumenActual,Diametro,Largo,Cantidad,Lote,Ano,Area,Registro,Propietario,Proveedor,Conductor,Placa,Cliente,FSC,Fecha,Hora,Maquinaria) VALUES (@Orden,@Clasificacion,@Motivo,@Especie,@volumenIngreso,@volumenActual,@Diametro,@Largo,@Cantidad,@Lote,@Ano,@Area,@Registro,@Propietario,@Proveedor,@Conductor,@Placa,@Cliente,@FSC,@Fecha,@Hora,@Maquinaria)");
            cmd.Connection = conn;
            conn.Open();
            if (conn.State == ConnectionState.Open)
            {
                ...
                cmd.Parameters.Add("@volumenIngreso", OleDbType.VarChar).Value = textBox38.Text;
                cmd.Parameters.Add("@volumenActual", OleDbType.VarChar).Value = textBox38.Text;
        ...
        public void modificarRecibo(string recibo)
        {
            ...
            if (conn.State == ConnectionState.Open)
            {
                ...
                if (radioButton18.Checked)
                    ...
                    cmd.Parameters.Add("@Especie", OleDbType.VarChar).Value = textBox36.Text;
                cmd.Parameters.Add("@volumenIngreso", OleDbType.VarChar).Value = textBox38.Text;
                cmd.Parameters.Add("@volumenActual", OleDbType.VarChar).Value = textBox5.Text;
                cmd.Parameters.Add("@Diametro", OleDbType.VarChar).Value = textBox41.Text;
                cmd.Parameters.Add("@Largo", OleDbType.VarChar).Value = textBox40.Text;
                cmd.Parameters.Add("@Cantidad", OleDbType.VarChar).Value = textBox39.Text;
                cmd.Parameters.Add("@Lote", OleDbType.VarChar).Value = comboBox1.SelectedValue;
