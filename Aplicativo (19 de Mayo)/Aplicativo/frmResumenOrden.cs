...
namespace Aplicativo
{
    public partial class frmResumenOrden : Form
    {
        ...
        public frmResumenOrden(int orden)
        {
            ...
            if (sw2)
            {
                ...
                for (int i = 0; i < tabControl1.TabPages.Count; i++)
                {
                    ...
                    {
                        cargarADF(dataGridView21, orden, "ADF019");
                        cargarADFCantidad19(dataGridView21, orden, "ADF019");
                        dataGridView21.Columns[dataGridView21.Columns.Count - 1].DefaultCellStyle.Format = "c";
                        label21.Text = "Total: " + String.Format("{0:c}",double.Parse(getTotal(dataGridView21, 6)));
                    }
                    if (Variables.tipo != 1)
                    {
                ...
                try
                {
                    while (myReader.Read())
                    {
                        if (!myReader.GetValue(0).ToString().Equals(""))
                        {
                            ...
                            var cultureInfo = new System.Globalization.CultureInfo("en-US");
                            double valor = 0;
                            if(data.Rows[i].Cells[6].Value != null)
                            valor = double.Parse(data.Rows[i].Cells[6].Value.ToString(), System.Globalization.NumberStyles.Currency);
                            data.Rows[i].Cells[7].Value = valor * double.Parse(myReader.GetValue(0).ToString());
                        }
                    }
                }
        ...
        public void cargarADFCantidad19(DataGridView data, int orden, string adf)
        {
            int index = 0;
            if (adf.Equals("ADF006"))
            {
                ...
                try
                {
                    while (myReader.Read())
                    {
                        if (!myReader.GetValue(0).ToString().Equals(""))
                        {
                            data.Rows[i].Cells[5].Value = myReader.GetValue(0).ToString();
                            var cultureInfo = new System.Globalization.CultureInfo("en-US");
                            double valor = double.Parse(data.Rows[i].Cells[4].Value.ToString(), System.Globalization.NumberStyles.Currency);
                            data.Rows[i].Cells[6].Value = valor * double.Parse(myReader.GetValue(0).ToString());
                        }
                    }
                }
                finally
        ...
        public void imprimirResumen()
        {
            ...
            for (int i = 0; i < tabControl1.TabPages.Count; i++)
            {
                ...
                {
                    imprimirFormato1(dataGridView21, "ADF019", XcelApp, label21.Text);
                }
            }
        }
