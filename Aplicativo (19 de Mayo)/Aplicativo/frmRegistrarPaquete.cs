...
namespace Aplicativo
{
    public partial class frmRegistrarPaquete : Form
    {
        ...
        public void agregarPaqueteParcial(int id)
        {
            conn.ConnectionString = connectionString;
            OleDbCommand cmd = new OleDbCommand("INSERT INTO paqueteParcial (num,op,Dia,Porcentaje,Producto) VALUES (@num,@op,@Dia,@Porcentaje,@Producto)");
            cmd.Connection = conn;
            conn.Open();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (numEntradas < 5)
            {
                if (radioButton2.Checked)
                {
                    if (!sw)
                    {
                        if (double.Parse(textBox13.Text) >= double.Parse(textBox1.Text))
                        {
                            int id = getMaxID();
                            ...
                            MessageBox.Show("Paquete Registrado.\nPaquete #: " + "P - " + nombre + " - " + (id + 1).ToString().PadLeft(4, '0') + "\nBodega #: " + comboBox2.Text);
                            this.Close();
                        }
                        else
                        {
                            MessageBox.Show("El paquete parcial tiene que ser menor que uno completo.");
                        }
                    }
                    else
            ...
            else
            {
                ...
                else
                {
                    if (!comboBox2.Text.Equals(""))
                    {
                        if (double.Parse(numPiezas) >= double.Parse(textBox1.Text))
                        {
                            modificarRegistro(paquete);
                            agregarAnchoAlto(paquete, dataGridView1, entradas);
                            int id = getMaxID();
                            agregarPaqueteParcial(id);
                            MessageBox.Show("Paquete terminado.");
                            this.Close();
                        }
                        else
                        {
                            MessageBox.Show("El # de piezas tiene que ser igual o menor a " + numPiezas);
                        }
                    }
                    else
                    {
