...
namespace Aplicativo
{
    public partial class frmActividades : Form
    {
        ...
        public void getInsumos(string id)
        {
            suma = 0;
            reiniciarTabler();
            string query = "SELECT i.ID, i.Insumo, a.ID,(a.Clase + ' ' + a.Marca + ' ' + a.Modelo) As Insumo, a.Costo_Unitario, i.Cantidad FROM Insumos AS a INNER JOIN insumoActividades AS i ON a.ID = i.Actividad WHERE i.Insumo = " + id;
            //Ejecutar el query y llenar el GridView.
            conn.ConnectionString = connectionString;
            OleDbCommand cmd = new OleDbCommand(query, conn);
            cmd.Connection = conn;
            ...
            try
            {
                while (myReader.Read())
                {
                    ...
                    dataGridView2.Rows[dataGridView2.Rows.Count - 1].Cells[1].Value = myReader.GetValue(3).ToString();
        ...
        public void getInsumo(string id) {            
            string query = "SELECT ID,(Clase + ' ' + Marca + ' ' + Modelo) As Insumo, Costo_Unitario FROM Insumos WHERE ID = " + id;
            //Ejecutar el query y llenar el GridView.
            try
            {
                conn.ConnectionString = connectionString;
