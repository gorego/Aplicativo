...
namespace Aplicativo
{
    public partial class frmProcesamientoFormatos : Form
    {
        ...
        public frmProcesamientoFormatos(int op, int dia, int tipo)
        {
            ...
            orden = op;
            tipo2 = tipo;
            estadoFSC = getEstadoFSC(op);
            formatoADF0202(dataGridView28, ADF020);
            formatoADF0202(dataGridView1, ADF020);
            ...
            if (getInfoMaquina("Monta Carga") != 0)
            {
                ...
                dataGridView8.Rows[0].Cells[3].Value = getInfoMaquina("Monta Carga");
            }
            ...
            if (getInfoMaquina("Chipeadora") != 0)
            {
                ...
                dataGridView11.Rows[0].Cells[3].Value = getInfoMaquina("Chipeadora");
            }
        ...
        public int getInfoMaquinaModelo(string tipo)
        {
            string query = "SELECT ID FROM Maquinarias WHERE Modelo = '" + tipo + "'";
            //Ejecutar el query y llenar el GridView.
            conn.ConnectionString = connectionString;
            OleDbCommand cmd = new OleDbCommand(query, conn);
            cmd.Connection = conn;
        ...
        public void seleccionarModeloADF020(DataGridView data)
        {
            for (int i = 0; i < data.Rows.Count; i++)
            {
                int id = getInfoMaquinaModelo(data.Rows[i].Cells[2].Value.ToString());
                if ( id != 0)
                    data.Rows[i].Cells[3].Value = id;
            }
        }
        ...
        public void cargarFormatoInput(DataGridView data)
        {
            ...
            data.Columns.Add("Column6", "Modulo");
            data.Columns.Add("Column7", "FSC");
        ...
        {
            string query = "SELECT r.Id,r.volumenActual, r.Motivo, r.Diametro, r.Largo, r.Cantidad, r.Modulo, r.numRecibo, l.Lote, l.estadoFSC FROM Recibo AS r INNER JOIN Lotes AS l ON r.Lote = l.Codigo WHERE r.Especie = '" + especie + "' AND (Month(Fecha) BETWEEN " + (DateTime.Now.Month - 3) + " AND " + (DateTime.Now.Month) + ")";
            if(estadoFSC.Equals("Si"))
                query = "SELECT r.Id,r.volumenActual, r.Motivo, r.Diametro, r.Largo, r.Cantidad, r.Modulo, r.numRecibo, l.Lote, l.estadoFSC FROM Recibo AS r INNER JOIN Lotes AS l ON r.Lote = l.Codigo WHERE r.Especie = '" + especie + "' AND (Month(Fecha) BETWEEN " + (DateTime.Now.Month - 3) + " AND " + (DateTime.Now.Month) + ") AND r.FSC <> '0'";
            //Ejecutar el query y llenar el GridView.
            conn.ConnectionString = connectionString;
            OleDbCommand cmd = new OleDbCommand(query, conn);
            cmd.Connection = conn;
            ...
            try
            {
                while (myReader.Read())
                {
                    ...
                    data.Rows[i].Cells[9].Value = myReader.GetString(9);
                    i++;
                }
            }
            finally
        ...
        {
            string query = "SELECT Paquete.Id, Paquete.numPaquete, Paquete.volumenActual, Paquete.Bodega, historicoProduccion.estadoFSC FROM (Paquete INNER JOIN Productos ON Paquete.Producto = Productos.ID) INNER JOIN historicoProduccion ON Paquete.OP = historicoProduccion.Id WHERE Productos.Caracteristica = '" + especie + "'";
            //Ejecutar el query y llenar el GridView.
            conn.ConnectionString = connectionString;
            OleDbCommand cmd = new OleDbCommand(query, conn);
            cmd.Connection = conn;
            ...
            try
            {
                while (myReader.Read())
                {
                    ...
                    data.Rows[i].Cells[9].Value = myReader.GetString(4);
                    i++;
                }
            }
            finally
        ...
        public void cargarPaquetesTotales(DataGridView data)
        {
            ...
            cargarPaquetesTotalesParciales(data,4);
        }

        public void cargarPaquetesTotalesParciales(DataGridView data, int indice)
        {
            for (int i = 0; i < data.Rows.Count; i++)
            {
                string query = "SELECT SUM(porcentaje) FROM paqueteParcial WHERE OP = " + orden + " AND Producto = " + data.Rows[i].Cells[1].Value;
                //Ejecutar el query y llenar el GridView.
                conn.ConnectionString = connectionString;
                OleDbCommand cmd = new OleDbCommand(query, conn);
                cmd.Connection = conn;
                ...
                try
                {
                    if (myReader.Read())
                    {
                        ...
                        if (myReader.GetValue(0).ToString().Equals(""))
                            ...
                            value = double.Parse(myReader.GetValue(0).ToString());
                        data.Rows[i].Cells[indice].Value = double.Parse(data.Rows[i].Cells[indice].Value.ToString()) + value;
                    }
                }
                finally
                {
        ...
        public void cargarPaquetesTotalesParcialesDiarios(DataGridView data, int indice)
        {
            for (int i = 0; i < data.Rows.Count; i++)
            {
                string query = "SELECT SUM(Porcentaje) FROM paqueteParcial WHERE OP = " + orden + " AND Producto = " + data.Rows[i].Cells[1].Value + " AND Dia = " + diaPaquete;
                //Ejecutar el query y llenar el GridView.
                conn.ConnectionString = connectionString;
                OleDbCommand cmd = new OleDbCommand(query, conn);
                cmd.Connection = conn;
                ...
                try
                {
                    if (myReader.Read())
                    {
                        if (!myReader.GetValue(0).ToString().Equals(""))
                        {
                            data.Rows[i].Cells[indice].Value = double.Parse(data.Rows[i].Cells[indice].Value.ToString()) + double.Parse(myReader.GetValue(0).ToString());
                            data.Rows[i].Cells[7].Value = double.Parse(data.Rows[i].Cells[6].Value.ToString()) * double.Parse(data.Rows[i].Cells[indice].Value.ToString());
                        }
                    }
                }
                finally
        ...
        public void cargarPaquetesTotalesDiarios(DataGridView data, int dia)
        {
            for (int i = 0; i < data.Rows.Count; i++)
            {
                string query = "SELECT SUM(porcentaje) FROM Paquete WHERE OP = " + orden + " AND Producto = " + data.Rows[i].Cells[1].Value + " AND dia = " + dia + " AND Parcial = 0";
                //Ejecutar el query y llenar el GridView.
                conn.ConnectionString = connectionString;
                OleDbCommand cmd = new OleDbCommand(query, conn);
                cmd.Connection = conn;
            ...
            cargarPaquetesTotalesParcialesDiarios(data,5);
        }
        ...
        public void cargarDetalleADF020(DataGridView data)
        {
            DataGridViewComboBoxCell combo = (DataGridViewComboBoxCell)(data.Rows[0].Cells[3]);
            //Ejecutar el query y llenar el ComboBox.
            ...
            for (int i = 0; i < data.Rows.Count; i++)
            {
                ...
                //Ejecutar el query y llenar el ComboBox.
                conn.ConnectionString = connectionString;
                cmd = new OleDbCommand("SELECT ID, (Modelo + ' / ' + Marca + ' / ' + Placa) As Maquina FROM Maquinarias WHERE Modelo = '" + data.Rows[i].Cells[2].Value + "'", conn);
                maquinaria = new DataTable();
                da = new OleDbDataAdapter(cmd);
                ds = new DataSet();
                da.Fill(ds);
        ...
        public void formatoADF0202(DataGridView data, string[,] formatos)
        {
            DataGridViewComboBoxColumn combo = new DataGridViewComboBoxColumn();
            combo.HeaderText = "Modelo";
            data.Columns.Add("Column1", "#");
            ...
            data.Columns[3].FillWeight = 300;
            formato2(data, formatos);
            cargarDetalleADF020(data);
            seleccionarModeloADF020(data);
        }
        ...
        public string getEstadoFSC(int op)
        {
            string query = "SELECT estadoFSC FROM historicoProduccion WHERE id = " + op;
            //Ejecutar el query y llenar el GridView.
            conn.ConnectionString = connectionString;
            OleDbCommand cmd = new OleDbCommand(query, conn);
            cmd.Connection = conn;
        ...
        {
            Contador(dataGridView28, 5, 17, ADF020);
        }
        ...
        {
            Contador(dataGridView1, 5, 17, ADF020);
        }
        ...
        public void modificarADFBaja(int semana, string orden, string adf, DataGridView data, int ano)
        {
            for (int i = 0; i < data.Rows.Count; i++)
            {
                conn.ConnectionString = connectionString;
                OleDbCommand cmd;
                cmd = new OleDbCommand("UPDATE ControlOP SET Lunes=@Lunes,Martes=@Martes,Miercoles=@Miercoles,Jueves=@Jueves,Viernes=@Viernes,Sabado=@Sabado WHERE Semana = " + semana + " AND Orden = " + orden + " AND Detalle = '" + data.Rows[i].Cells[2].Value.ToString() + "' AND ADF = '" + adf + "' AND Ano = " + ano);
                cmd.Connection = conn;
                conn.Open();
                if (conn.State == ConnectionState.Open)
                {                    
                    cmd.Parameters.Add("@Lunes", OleDbType.VarChar).Value = data.Rows[i].Cells[4].Value.ToString();
                    cmd.Parameters.Add("@Martes", OleDbType.VarChar).Value = data.Rows[i].Cells[6].Value.ToString();
                    cmd.Parameters.Add("@Miercoles", OleDbType.VarChar).Value = data.Rows[i].Cells[8].Value.ToString();
                    cmd.Parameters.Add("@Jueves", OleDbType.VarChar).Value = data.Rows[i].Cells[10].Value.ToString();
                    cmd.Parameters.Add("@Viernes", OleDbType.VarChar).Value = data.Rows[i].Cells[12].Value.ToString();
                    cmd.Parameters.Add("@Sabado", OleDbType.VarChar).Value = data.Rows[i].Cells[14].Value.ToString();
                    try
                    {
                        cmd.ExecuteNonQuery();
                        conn.Close();
        ...
        public void crearADFBaja(int semana, string orden, string adf, DataGridView data, int ano)
        {
            for (int i = 0; i < data.Rows.Count; i++)
            {
                conn.ConnectionString = connectionString;
                ...
                {
                    cmd.Parameters.Add("@Semana", OleDbType.VarChar).Value = semana;
                    cmd.Parameters.Add("@Unidad", OleDbType.VarChar).Value = data.Rows[i].Cells[3].Value.ToString();
                    cmd.Parameters.Add("@Orden", OleDbType.VarChar).Value = orden;
                    cmd.Parameters.Add("@Detalle", OleDbType.VarChar).Value = data.Rows[i].Cells[2].Value.ToString();
                    cmd.Parameters.Add("@Modelo", OleDbType.VarChar).Value = 0;
                    cmd.Parameters.Add("@Lunes", OleDbType.VarChar).Value = data.Rows[i].Cells[4].Value.ToString();
        ...
        public void cargarADF020(int semana, string orden, string adf, DataGridView data, int ano)
        {
            for (int i = 0; i < data.Rows.Count; i++)
            {
                string query = "SELECT * FROM ControlOP WHERE Semana = " + semana + " AND Orden = " + orden + " AND ADF = '" + adf + "' AND Ano = " + ano;
                ...
                try
                {
                    while (myReader.Read())
                    {
                        for (int j = 0; j < 5; j++)
                        {
                            data.Rows[i].Cells[(j * 2) + 4].Value = myReader.GetDouble(j + 6);
                        }
                    }
                }
                finally
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            if (ADFExiste(semana, orden.ToString(), "ADF006-Mini Cargador", ano) == false)
            {
                ...
                controlCosto(orden, getControlCosto(semana, orden, "ADF006-Mini Cargador"), "+");            
            }
            else
            {
                controlCosto(orden, getControlCosto(semana, orden, "ADF006-Mini Cargador"), "-"); 
        ...
        public void controlCosto(int orden, double costo, string simbolo)
        {
            conn.ConnectionString = connectionString;
            OleDbCommand cmd;
            cmd = new OleDbCommand("UPDATE historicoProduccion SET costo = costo " + simbolo + " @costo WHERE ID = " + orden);
            cmd.Connection = conn;
            conn.Open();
            if (conn.State == ConnectionState.Open)
            {
        ...
        public double getControlCosto(int semana, int orden, string adf)
        {
            ...
            string query = "SELECT  SUM((c.Lunes+c.Martes+c.Miercoles+c.Jueves+c.Viernes+c.Sabado)*i.Costo_Unitario) FROM (ControlOP AS c INNER JOIN historicoProduccion AS h ON c.Orden = h.ID) INNER JOIN Insumos AS i ON c.Modelo = i.ID WHERE h.ID = " + orden + " AND c.Semana = " + semana + " AND c.ADF = '" + adf + "'";
            //Ejecutar el query y llenar el GridView.
            conn.ConnectionString = connectionString;
            OleDbCommand cmd = new OleDbCommand(query, conn);
            cmd.Connection = conn;
        ...
        private void linkLabel6_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            label59.Text = "Día #: " + (diaPaquete + 1) + "                     Fecha: " + date.ToString("dd") + "/" + date.ToString("MM") + "/" + date.Year; 
            cargarPaquetesTotales(dataGridView14);
            cargarPaquetesTotalesDiarios(dataGridView14, diaPaquete);
        }
        ...
        private void linkLabel1_LinkClicked_1(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            clearDataGrid(dataGridView28);
            clearDataGrid(dataGridView1);
            clearDataGrid(dataGridView2);
            formatoADF0202(dataGridView28, ADF020);
            formatoADF0202(dataGridView1, ADF020);
            formatoADF020(dataGridView2, ADF0201);
            cargarADF(semana, orden.ToString(), "ADF020", dataGridView28, ano, "");
            cargarADF(semana, orden.ToString(), "ADF020-3", dataGridView1, ano, "");
            cargarADF020(semana, orden.ToString(), "ADF020-2", dataGridView2, ano);
        }
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            if (ADFExiste(semana, orden.ToString(), "ADF006-Monta Carga", ano) == false)
            {
                ...
                controlCosto(orden, getControlCosto(semana, orden, "ADF006-Monta Carga"), "+");            
            }
            else
            {
                controlCosto(orden, getControlCosto(semana, orden, "ADF006-Monta Carga"), "-");            
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (ADFExiste(semana, orden.ToString(), "ADF006-Chipeadora", ano) == false)
            {
                ...
                controlCosto(orden, getControlCosto(semana, orden, "ADF006-Chipeadora"), "+");            
            }
            else
            {
                controlCosto(orden, getControlCosto(semana, orden, "ADF006-Chipeadora"), "-");            
        ...
        {
            if (ADFExiste(semana, orden.ToString(), "ADF020", ano) == false)
            {
                crearADF(semana, orden.ToString(), "ADF020", dataGridView28, ano, "");
                crearADF(semana, orden.ToString(), "ADF020-3", dataGridView1, ano, "");
                crearADFBaja(semana, orden.ToString(), "ADF020-2", dataGridView2, ano);
            }
            else
            {
                modificarADF(semana, orden.ToString(), "ADF020", dataGridView28, ano, "");
                modificarADF(semana, orden.ToString(), "ADF020-3", dataGridView1, ano,"");
                modificarADFBaja(semana, orden.ToString(), "ADF020-2", dataGridView2, ano);
