    ...
    public partial class frmVerMaquinaria : Form
    {
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            if (!textBox1.Text.Equals("") && !textBox2.Text.Equals(""))
            {
                ...
                for (int i = 0; i < dataGridView5.Rows.Count; i++)
                {
                    costo += double.Parse(dataGridView5.Rows[i].Cells[0].Value.ToString());
                }
        ...
        {
            string query = "SELECT ID,(Clase + ' ' + Marca + ' ' + Modelo) As Insumo FROM Insumos";
        ...
        {
            string query = "SELECT ID,(Clase + ' ' + Marca + ' ' + Modelo),Costo_Unitario FROM Insumos WHERE ID = " + id;
            //Ejecutar el query y llenar el GridView.
            try
            {
                conn.ConnectionString = connectionString;
                ...
                if (reader.Read())
                {
                    ...
                    textBox2.Text = reader.GetDouble(2).ToString();
                    textBox4.Text = reader.GetDouble(2).ToString();
                }
            }
            catch (Exception ex)
            {
