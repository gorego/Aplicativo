...
namespace Aplicativo
{
    public partial class frmHistoricoProduccion : Form
    {
        ...
        public frmHistoricoProduccion(int tipo)
        {
            ...
            {
                Variables.cargar(dataGridView1, "SELECT historicoProduccion.ID, historicoProduccion.OP, historicoProduccion.Tipo, historicoProduccion.Especie, historicoProduccion.fechaInicio, historicoProduccion.horaInicio, historicoProduccion.fechaFinal, Sum(produccionProducto.Volumen) AS SumaDeVolumen, historicoProduccion.estadoFSC, Clientes.Cliente, historicoProduccion.Estado FROM (historicoProduccion INNER JOIN produccionProducto ON historicoProduccion.Id = produccionProducto.Orden) INNER JOIN Clientes ON historicoProduccion.Cliente = Clientes.ID GROUP BY historicoProduccion.ID, historicoProduccion.OP, historicoProduccion.Tipo, historicoProduccion.Especie, historicoProduccion.fechaInicio, historicoProduccion.horaInicio, historicoProduccion.fechaFinal, historicoProduccion.estadoFSC, Clientes.Cliente, historicoProduccion.Estado ORDER BY historicoProduccion.ID DESC;");
        ...
        public void estadoOrdenes(int tipo)
        {
            if (tipo == 0)
            {
                for (int i = 0; i < dataGridView1.Rows.Count; i++)
                {
                    ...
                    if (DateTime.Compare(DateTime.Now.Date, fechaFinal.Date) > 0 && !dataGridView1.Rows[i].Cells[10].Value.Equals("Cerrada"))
                    {
                        dataGridView1.Rows[i].Cells[10].Value = "Vencida";
                    }
                }
            }
            else
            {
                for (int i = 0; i < dataGridView2.Rows.Count; i++)
                {
                    ...
                    if (DateTime.Compare(DateTime.Now.Date, fechaFinal.Date) > 0 && !dataGridView2.Rows[i].Cells[10].Value.Equals("Cerrada"))
                    {
                        dataGridView2.Rows[i].Cells[10].Value = "Vencida";
                    }
                }
            }
        }
        ...
        {
            frmHistoricoOrdenes newFrm = new frmHistoricoOrdenes(Variables.userID, 1, Variables.userName);
            newFrm.Show();
        }
    }
}
