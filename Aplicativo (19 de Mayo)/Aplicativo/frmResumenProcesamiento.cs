...
namespace Aplicativo
{
    public partial class frmResumenProcesamiento : Form
    {
        ...
        public frmResumenProcesamiento(int orden, int dia, int tipo)
        {
            ...
            dataGridView13.Columns[8].DefaultCellStyle.Font = new Font(dataGridView13.DefaultCellStyle.Font, FontStyle.Underline);
            ...
            label30.Text = (Math.Round(Math.Round(double.Parse(dataGridView14.Rows[dataGridView14.Rows.Count - 1].Cells[dataGridView14.Columns.Count - 1].Value.ToString()),3,MidpointRounding.AwayFromZero) / double.Parse(label23.Text.Replace("Volumen total seleccionado:", "").Replace(" m3.","")) * 100,3,MidpointRounding.AwayFromZero) + "%").Replace(",",".");
        }
        ...
        public void cargarPaquetesTotales(DataGridView data)
        {
            for (int j = 0; j < idProductos.Count; j++)
            {
                for (int i = 0; i < data.Rows.Count; i++)
                {
                    string query = "SELECT SUM(porcentaje) FROM Paquete WHERE OP = " + op + " AND dia = " + (Int32.Parse(data.Rows[i].Cells[0].Value.ToString()) - 1) + " AND Parcial = 0 AND Producto = " + idProductos[j] + " UNION ALL SELECT SUM(porcentaje) FROM paqueteParcial WHERE OP = " + op + " AND dia = " + (Int32.Parse(data.Rows[i].Cells[0].Value.ToString()) - 1) + " AND Producto = " + idProductos[j];
                    ...
                    try
                    {
                        while (myReader.Read())
                        {
                            ...
                            if (data.Rows[i].Cells[1 + (j * 2)].Value != null)
                                test = double.Parse(data.Rows[i].Cells[1 + (j * 2)].Value.ToString());
                            data.Rows[i].Cells[1 + (j*2)].Value = test + value;
                        }
                    }
                    finally
                    {
        ...
        {
            double total = 0;
            string query = "SELECT recibosOrdenes.Dia, Recibo.numRecibo, Lotes.Lote, recibosOrdenes.Porcentaje, recibosOrdenes.volumen, Recibo.Modulo, Lotes.estadoFSC, h.OT, h.ID FROM ((recibosOrdenes INNER JOIN Recibo ON recibosOrdenes.Recibo = Recibo.Id) INNER JOIN Lotes ON Recibo.Lote = Lotes.Codigo) INNER JOIN historicoOrdenes AS h ON Recibo.Orden = h.ID WHERE (((recibosOrdenes.OP)= " + orden + ")) ORDER BY recibosOrdenes.Dia;";
            //Ejecutar el query y llenar el GridView.
            conn.ConnectionString = connectionString;
            OleDbCommand cmd = new OleDbCommand(query, conn);
            cmd.Connection = conn;
            ...
            try
            {
                while (myReader.Read())
                {
                    ...
                    data.Rows[i].Cells[8].Value = myReader.GetString(7);
                    data.Rows[i].Cells[9].Value = myReader.GetInt32(8);
        ...
        {
            double total = 0;
            string query = "SELECT paqueteOrdenes.Dia, Paquete.numPaquete, paqueteOrdenes.Porcentaje, paqueteOrdenes.volumen, Paquete.Bodega, h.OP, h.Id, h.estadoFSC FROM (historicoProduccion AS h INNER JOIN Paquete ON h.Id = Paquete.OP) INNER JOIN paqueteOrdenes ON Paquete.Id = paqueteOrdenes.Paquete WHERE paqueteOrdenes.OP = " + orden + " ORDER BY paqueteOrdenes.Dia;";
            //Ejecutar el query y llenar el GridView.
            conn.ConnectionString = connectionString;
            OleDbCommand cmd = new OleDbCommand(query, conn);
            cmd.Connection = conn;
            ...
            try
            {
                while (myReader.Read())
                {
                    ...
                    data.Rows[i].Cells[7].Value = myReader.GetString(7);
                    data.Rows[i].Cells[8].Value = myReader.GetString(5);
                    data.Rows[i].Cells[9].Value = myReader.GetInt32(6);
        ...
        {
            if (dataGridView13.CurrentCell.ColumnIndex == 8)
            {
                if (dataGridView13.Rows[dataGridView13.CurrentCell.RowIndex].Cells[8].Value.ToString().Contains("OT"))
                {
                    frmCrearOrden newFrm = new frmCrearOrden(dataGridView13.Rows[dataGridView13.CurrentCell.RowIndex].Cells[9].Value.ToString(), 1);
                    newFrm.Show();
                }
                else
                {
                    frmCrearProduccion newFrm = new frmCrearProduccion(Int32.Parse(dataGridView13.Rows[dataGridView13.CurrentCell.RowIndex].Cells[9].Value.ToString()), 1);
                    newFrm.Show();
                }
            }
        }
