...
namespace Aplicativo
{
    public partial class frmCrearProduccion : Form
    {
        ...
        public frmCrearProduccion(int orden, int tipo)
        {
            ...
            getProductos(orden);
            getVolProd();
            double inputs = getInputs(orden);
            if (inputs != 0)
                txtAprovechamiento.Text = ((Math.Round(getVolPaquete(orden) / inputs * 100, 3, MidpointRounding.AwayFromZero) + "%").Replace(",", "."));
            else
                txtAprovechamiento.Text = "100%";
        }

        public double getInputs(int orden)
        {
            double i = 0;
            string query = "SELECT SUM(volumen) FROM recibosOrdenes WHERE OP = " + orden + " UNION ALL SELECT SUM(volumen) FROM paqueteOrdenes WHERE OP = " + orden;
            ...
            try
            {
                while (myReader.Read())
                {
                    if(!myReader.GetValue(0).ToString().Equals(""))
                        i += Convert.ToDouble(myReader.GetValue(0).ToString());
                }
            }
            finally
            {
        ...
        public double getVolPaquete(int orden)
        {
            string query = "SELECT Sum(Paquete.Porcentaje) AS SumaDePorcentaje, Productos.Codigo, ((Productos.anchoProd*Productos.altoProd*Productos.largoProd)/1000000000)*(Productos.numAnchoEmp*numAltoEmp)*(Sum(Paquete.Porcentaje)) AS Expr1 FROM Productos INNER JOIN Paquete ON Productos.ID = Paquete.Producto WHERE (((Paquete.OP)="+orden+")) GROUP BY Productos.Codigo, ((Productos.anchoProd*Productos.altoProd*Productos.largoProd)/1000000000)*(Productos.numAnchoEmp*numAltoEmp);";
            //Ejecutar el query y llenar el GridView.
            conn.ConnectionString = connectionString;
            OleDbCommand cmd = new OleDbCommand(query, conn);
            cmd.Connection = conn;
            ...
            try
            {
                while (myReader.Read())
                {
                    i += Convert.ToDouble(myReader.GetValue(2).ToString());
                }
            }
            finally
            {
        ...
        public void getOrden(int orden)
        {
            ...
            try
            {
                if (myReader.Read())
                {
                    ...
                    txtCostoTotal.Text = String.Format("{0:c}",myReader.GetDouble(12));
        ...
        public void getVolProd()
        {
            double Total = 0;
            for (int i = 0; i < dataGridView1.Rows.Count; i++)
            {
                Total += Convert.ToDouble(dataGridView1.Rows[i].Cells[dataGridView1.Columns.Count - 1].Value);
            }

            label14.Text = "Total Vol. a Prod: " + Total.ToString() + " m3.";
        }
        ...
        {
            conn.ConnectionString = connectionString;
            OleDbCommand cmd = new OleDbCommand("INSERT INTO historicoProduccion (fechaExpedicion,fechaInicio,horaInicio,fechaFinal,Tipo,Especie,Cliente,Nacional,FSC,Estado,OP,estadoFSC) VALUES (@fechaExpedicion,@fechaInicio,@horaInicio,@fechaFinal,@Tipo,@Especie,@Cliente,@Nacional,@FSC,@Estado,@OP,@estadoFSC)");
            cmd.Connection = conn;
            conn.Open();
            if (conn.State == ConnectionState.Open)
            {
                ...
                if (radioButton13.Checked)
                {
                    ...
                    cmd.Parameters.Add("@estadoFSC", OleDbType.VarChar).Value = "Si";
                }
                else
                {
                    ...
                    cmd.Parameters.Add("@Estado", OleDbType.VarChar).Value = "Activa";
                    cmd.Parameters.Add("@OP", OleDbType.VarChar).Value = "OP-" + id.ToString().PadLeft(4, '0') + "-" + DateTime.Now.Year;
                    cmd.Parameters.Add("@estadoFSC", OleDbType.VarChar).Value = "No";
                }
                try
                {
                    cmd.ExecuteNonQuery();
        ...
        {
            conn.ConnectionString = connectionString;
            OleDbCommand cmd = new OleDbCommand("UPDATE historicoProduccion SET fechaInicio=@fechaInicio,horaInicio=@horaInicio,fechaFinal=@fechaFinal,Tipo=@Tipo,Especie=@Especie,Cliente=@Cliente,Nacional=@Nacional,FSC=@FSC,estadoFSC=@estadoFSC WHERE ID = " + id);
            cmd.Connection = conn;
            conn.Open();
            if (conn.State == ConnectionState.Open)
            {
        ...
        public void imprimirOP()
        {
            ...
            string[] prueba = Directory.GetFiles("C:\\Users\\" + Environment.UserName + "\\Dropbox\\Aplicativo\\Formatos\\", "OP*");
