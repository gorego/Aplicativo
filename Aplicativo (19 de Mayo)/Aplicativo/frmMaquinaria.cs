...
namespace Aplicativo
{
    public partial class frmMaquinaria : Form
    {
        ...
        public void buscarMaquinaria()
        {
            ...
            if (!txtNumPuestos.Text.Equals("") && !txtNumPuestos.Text.Equals("0"))
            {
                if (i != 0)
                    query += " AND ";
                else
                    query += "WHERE ";
                i++;
                query += "m.numPuestos = " + txtNumPuestos.Text;
            }
            if (!txtEstado.Text.Equals(""))
            {
                if (i != 0)
                    ...
                    query += "WHERE ";
                i++;
                query += "m.Estado = " + txtEstado.SelectedValue;
            }
            if (!txtValor.Text.Equals("") && !txtValor.Text.Equals("0"))
            {
                if (i != 0)
                    query += " AND ";
                else
                    query += "WHERE ";
                i++;
                query += "m.valorHora = " + txtValor.Text;
            }
