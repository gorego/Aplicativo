...
namespace Aplicativo
{
    public partial class frmCrearOrden : Form
    {
        ...
        int tip, area, lote, actividad, tipousuario;
        int p = 0, a = 0, b = 0, c = 0;
        string user,unidad;
        ...
        {
            conn.ConnectionString = connectionString;
            OleDbCommand cmd = new OleDbCommand("INSERT INTO historicoOrdenes (fechaExpedicion,Contrato,Operador,Departamento,Supervisor,Actividad,fechaInicio,fechaFinal,Lote,Area,estadoLote,Centro,Costo,estadoOrden,CostoJornal,Descripcion,Cliente,Transportador,costoFinal,costoJornalFinal,OT) VALUES (@fechaExpedicion,@Contrato,@Operador,@Departamento,@Supervisor,@Actividad,@fechaInicio,@fechaFinal,@Lote,@Area,@estadoLote,@Centro,@Costo,@estadoOrden,@CostoJornal,@Descripcion,@Cliente,@Transportador,@costoFinal,@costoJornalFinal,@OT)");
            cmd.Connection = conn;
            conn.Open();
        ...
        public void getActividad(string Actividad)
        {
            ...
            try
            {
                while (myReader.Read())
                {
                    ...
                    costo = Int32.Parse(myReader.GetValue(3).ToString());
        ...
        public void getMaquinas(string id)
        {
            ...
            maquinarias.Clear();
            string query = "SELECT i.ID, i.Orden, (m.Tipo + ' /' + m.Marca + '/' + m.Placa) As Maquinaria, m.ID FROM Maquinarias AS m INNER JOIN ordenMaquinas AS i ON m.ID = i.Maquina WHERE i.Orden = " + id;
            //Ejecutar el query y llenar el GridView.
            conn.ConnectionString = connectionString;
            OleDbCommand cmd = new OleDbCommand(query, conn);
            cmd.Connection = conn;
