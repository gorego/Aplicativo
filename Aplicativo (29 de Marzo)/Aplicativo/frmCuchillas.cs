...
namespace Aplicativo
{
    public partial class frmCuchillas : Form
    {
        ...
        {
            InitializeComponent();
            Variables.cargar(comboBox1, "SELECT ID, (Placa + ' / ' + Marca + ' / ' + Modelo) As Maquina  FROM Maquinarias WHERE Tipo = 'Aserradero';", "Maquina");
            Variables.cargar(txtCuchilla, "SELECT ID, (Marca + ' / ' + Modelo) As Cuchilla  FROM Insumos WHERE Clase = 'Cuchilla';", "Cuchilla");
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            textBox4.Text = getPosturas(dataGridView1.Rows[dataGridView1.CurrentCell.RowIndex].Cells[0].Value.ToString());
        }
        ...
        public string getPosturas(string id)
        {
            string query = "SELECT COUNT(Hora) FROM cuchillaHoras WHERE Cuchilla = " + id;
            //Ejecutar el query y llenar el GridView.
            conn.ConnectionString = connectionString;
            OleDbCommand cmd = new OleDbCommand(query, conn);
            cmd.Connection = conn;
