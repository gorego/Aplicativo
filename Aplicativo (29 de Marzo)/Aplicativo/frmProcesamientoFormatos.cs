...
namespace Aplicativo
{
    public partial class frmProcesamientoFormatos : Form
    {
        ...
        {
            string query = "SELECT r.Id,r.volumenActual, r.Motivo, r.Diametro, r.Largo, r.Cantidad, r.Modulo, r.numRecibo, l.Lote FROM Recibo AS r INNER JOIN Lotes AS l ON r.Lote = l.Codigo WHERE r.Especie = '" + especie + "' AND (Month(Fecha) BETWEEN " + (DateTime.Now.Month - 3) + " AND " + (DateTime.Now.Month) + ")";
            //Ejecutar el query y llenar el GridView.
            conn.ConnectionString = connectionString;
            OleDbCommand cmd = new OleDbCommand(query, conn);
            cmd.Connection = conn;
