    ...
    public partial class frmDespacho : Form
    {
        ...
        public void cargar(DataGridView data, int ano)
        {
            if(ano != -1)
                ...
                Variables.cargar(data, "SELECT Despacho.despacho, Clientes.cliente, Transportadores.nombres, numICA, numICAWeb, FSC, Fecha, Hora, Tipo FROM (Despacho INNER JOIN Transportadores ON Despacho.Transportador = Transportadores.ID) INNER JOIN Clientes ON Despacho.Cliente = Clientes.ID ORDER BY Despacho.ID DESC;");
