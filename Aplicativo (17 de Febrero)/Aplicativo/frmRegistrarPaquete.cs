...
namespace Aplicativo
{
    public partial class frmRegistrarPaquete : Form
    {
        ...
        {
            conn.ConnectionString = connectionString;
            OleDbCommand cmd = new OleDbCommand("INSERT INTO Paquete (numPaquete,Producto,OP,Bodega,numPiezas,num,dia,porcentaje) VALUES (@numPaquete,@Producto,@OP,@Bodega,@numPiezas,@num,@dia,@porcentaje)");
            cmd.Connection = conn;
            conn.Open();
        ...
        {
            conn.ConnectionString = connectionString;
            OleDbCommand cmd = new OleDbCommand("UPDATE Paquete SET Bodega=@Bodega,numPiezas=numPiezas+@piezas,dia=@dia,porcentaje=porcentaje+@porcentaje2 WHERE ID = " + paquete);
            cmd.Connection = conn;
            conn.Open();
            if (conn.State == ConnectionState.Open)
            {
